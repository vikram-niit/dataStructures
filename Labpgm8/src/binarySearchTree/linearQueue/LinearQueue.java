...
public class LinearQueue
{
		...
		private BSTNode[] queue = new BSTNode[5];
		...
		public BSTNode dequeue() throws QueueEmptyException
		{
			if(isEmpty())
				throw new QueueEmptyException();
			
			BSTNode element = queue[front];
			queue[front] = null;
		...
		public void resize()
		{
			BSTNode[] newQueue = new BSTNode[queue.length << 1];
			...
			for(;;)
			{
				...
				switch(choice)
				{
					case 1: System.out.println("Enter the element to be inserted");
							...
							queue.enqueue(new BSTNode(element));
							System.out.println("After enqueuing "+queue);
							break;
							
					case 2: try{
							node = queue.dequeue();
							System.out.println("Deleted element = "+node);
